...
<%
	...
	double undigestion_times = 0.00;
	for(PaidHolidayBean phb : phList) {
		undigestion_times = phb.getUndigestionTimes();
	}
	int hour = (int)(undigestion_times);
	int min = (int)((undigestion_times * 60) % 60);
	String undigestionTime = null;
	if(min == 0) {
		undigestionTime = hour + ":" + min + "0";
	} else {
		undigestionTime = hour + ":" + min;
	}
%>
			...
			<div id="./various_applications_view.jsp" class="tab-content active">
				 <div class="div" align="center">
					 <form action="VacationRequestProcessServlet" method="post" name="vacateForm">
	                    ...
	                    <table class="table1">
	                        <tr class="table1_tr1">
	                            <td class="table1_tr1_td1" align="right" style="width: 50%;">申請内容　</td>
	                        ...
	                        <% if("年次有給休暇".equals(vacation_name)) { %>
	                        	<tr class="table1_tr2">
	                                <td class="table1_tr2_td1" align="right">申請方法　</td>
	                        ...
	                        <% if(!"特別時間休暇".equals(vacation_name)) { %>
	                        	<tr class="table1_tr3">
		                        	<td class="table1_tr3_td1" align="right">取得日　</td>
		                        	<% if("年次有給休暇".equals(vacation_name) && "複数日申請".equals(classification)) { %>
		                        		<td class="table1_tr3_td2">
		                                    <p class="table1_tr3_td2_p">注：一度に申請できる日数は、5日分までです。</p>
		                        	...
		                        	<% } else if("年次有給休暇".equals(vacation_name) && "範囲指定申請".equals(classification)) { %>
		                        		<td class="table1_tr3_td2">
		                        			...
		                        			<input type="date" name="date2" class="table1_tr3_td2_date">
		                        		</td>
		                        	<% } else { %>
		                        		<td class="table1_tr3_td2">
		                        			<input type="date" name="date1" class="table1_tr3_td2_date">
	                        ...
	                        <% if("特別時間休暇".equals(vacation_name)) { %>
	                        	<tr class="table1_tr4">
	                        		<td class="table1_tr4_td1" align="right">申請時間　</td>
	                        ...
	                        <% } %>
	                        <tr class="table1_tr5">
	                            <td class="table1_tr5_td1" align="right">申請理由　</td>
	                            <td class="table1_tr5_td2">
                                    <textarea name="reason" class="table1_tr5_td2_textarea" cols="50" rows="2"></textarea>
                                </td>
                            </tr>
	                        <tr class="table1_tr6">
	                            <td class="table1_tr6_td1" align="right">コメント　</td>
	                            <td class="table1_tr6_td2">
                                    <textarea name="remarks" class="table1_tr6_td2_textarea" cols="50" rows="2"></textarea>
                                </td>
                            </tr>
			                        ...
			                        <td class="table1_tr4_td1" colspan="2">
			                        	<% for(PaidHolidayBean phb : phList) { %>
			                        		<% if(phb.getPeriodUse() == null || phb.getPeriodUse().equals("")) { %>
			                        			<div class="table1_tr4_td1_div" style="text-align: center;">　　　有給休暇がまだ、付与されておりません。</div>
			                        		<% } else if("年次有給休暇".equals(vacation_name)){ %>
			                        			<div class="table1_tr4_td1_div" style="text-align: center;">残日数：<%= phb.getUndigestionDays() %>日</div>
			                        		<% } else { %>
			                        			<div class="table1_tr4_td1_div" style="text-align: center;">　　　残日数時間数：<%= phb.getUndigestionDays() %>日 <%= undigestionTime %></div>
			                        		<% } %>
			                        	<% } %>
			                        </td>
			                    </tr>
