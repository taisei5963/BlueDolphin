...
public class ApplicationApproverDAO{
	 ...
	 */
	public Integer getEmployeeNumber(String authorizer) throws SQLException {
		log.info("getEmployeeNumber start");
		
		//取得情報格納用変数宣言
		Integer num = null;
		
		//SQL文
		String sql = "select emp_num from employee where emp_name ='" + authorizer + "';";
		rs = st.executeQuery(sql);
		log.debug("st : {}", st);
		
		//SQL実行結果
		if(rs.next()) {
			num = rs.getInt("emp_num");
		} else {
			log.error(NOTDATA);
		}
		log.info("getEmployeeNumber end");
		return num;
	}
}
