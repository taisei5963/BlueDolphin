...
public class KintaiDAO{
	 ...
	 * @throws SQLException データベース処理に問題が発生した場合
	 */
	public ArrayList<KinmuTimeBean> getWorkTime(Integer emp_num) throws SQLException{
		log.info("getWorkTime start");
		
		//格納用リストのインスタンス生成
		ArrayList<KinmuTimeBean> kinmuTimeList = new ArrayList<KinmuTimeBean>();
		//日付・時刻の取得
		LocalDateTime now = LocalDateTime.now();
	...
	public ArrayList<KinmuAllTimeBean> getAllWorkTime(Integer emp_num, String thisMonth) throws SQLException{
		...
		while(rs.next()) {
			...
			if(rs.getString("actual_hours") != null) {
				ktBean.setActualTime(LocalTime.parse(rs.getString("actual_hours"), DateTimeFormatter.ofPattern("HH:mm:00")));
			} else if(rs.getString("actual_hours") == null) {
				ktBean.setActualTime(null);
			}
			if(rs.getString("overtime_hours") != null) {
				ktBean.setZangyoTime(LocalTime.parse(rs.getString("overtime_hours"), DateTimeFormatter.ofPattern("HH:mm:00")));
	...
	public String getScheduledWorkingHours(Integer emp_num) throws SQLException {
		...
		String scheduled_working_hours = null;
		...
		if(rs.next()) {
			scheduled_working_hours = rs.getString("scheduled_working_hours");
		}
		log.info("getStandardTime end");
		return scheduled_working_hours;
	}
	
	/**
	 * 従業員の実動時間の合計値を取得するメソッド
	 * @param emp_num　従業員番号
	 * @param emp_workday　合計値算出対象年月
	 * @return　実動時間合計値
	 * @throws SQLException　データベース処理に問題が発生した場合
	 */
	public String getTotalActualHours(Integer emp_num, String emp_workday) throws SQLException {
		log.info("getTotalActualHours start");
		
		//取得データ格納用変数宣言(NULLで初期化)
		String total_actual = null;
		
		//SQL文
		String sql = "select sec_to_time(sum(time_to_sec(actual_hours))) as total_actual_hours from emp_kintai "
					+ "where emp_num = '" + emp_num + "' and emp_workday like '" + emp_workday + "%';";
		
		rs = st.executeQuery(sql);
		log.debug(rs);
		
		if(rs.next()) {
			total_actual = rs.getString("total_actual_hours");
		} else {}
	...
	/**
	 * 従業員の残業時間の合計値を取得するメソッド
	 * @param emp_num　従業員番号
	 * @param emp_workday　合計値算出対象年月
	 * @return　残業時間合計値
	 * @throws SQLException　データベース処理に問題が発生した場合
	 */
	public String getTotalOvertimeHours(Integer emp_num, String emp_workday) throws SQLException {
		log.info("getTotalActualHours start");
		
		//取得データ格納用変数宣言(NULLで初期化)
		String total_overtime = null;
		
		//SQL文
		String sql = "select sec_to_time(sum(time_to_sec(overtime_hours))) as total_overtime_hours from emp_kintai "
					+ "where emp_num = '" + emp_num + "' and emp_workday like '" + emp_workday + "%';";
		...
		if(rs.next()) {
			total_overtime = rs.getString("total_overtime_hours");
		} else {}
