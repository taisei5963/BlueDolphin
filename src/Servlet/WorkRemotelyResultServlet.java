...
public class WorkRemotelyResultServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//申請処理
			...
			String mailAdd = null;
			ApplicationApproverDAO appdao = ApplicationApproverDAO.getInstance();
			
			//承認者情報取得
			try {
				appdao.dbConenect();
				appdao.createSt();
				authorizer = appdao.getAuthorizer(applicant);
				mailAdd = appdao.getAuthorizerMailAdd(authorizer);
			...
			//取得した情報のNULLチェック
			if(authorizer == null  || mailAdd == null) {
				log.error(NULLPOINTEREXCEPTION);
				return;
			}
			
			session.setAttribute("authorizer", authorizer);
